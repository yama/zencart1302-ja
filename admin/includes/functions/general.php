...
// error on 1969 only allows for leap year
    if (@date('Y', zen_mktime($hour, $minute, $second, $month, $day, $year)) == $year) {
      return date(DATE_FORMAT, zen_mktime($hour, $minute, $second, $month, $day, $year));
...
///////
// This mktime function support before 1970.
// Author: rickenmeer at hotmail dot com (http://jp.php.net/manual/ja/function.mktime.php)
  function zen_mktime($hour = false, $minute = false, $second = false, $month = false, $date = false, $year = false)
  {
       ...
       $days = 0 - $d_year * 365;
       $days -= ($d_year < 2   ? 0 :
                                 floor (($d_year -2  ) / 4)    + 1);  // compensate for leap-years
       $days += ($d_year < 70  ? 0 :
                                 floor (($d_year - 70) / 100)  + 1);  // compensate for non-leap-years
       $days -= ($d_year < 370 ? 0 :
                                 floor (($d_year - 370) / 400) + 1);  // compensate again for giant leap-years
  ...
  function zen_remove_category($category_id) {
    ...
    if ($duplicate_image->fields['total'] < 2) {
      if (file_exists(DIR_FS_CATALOG_IMAGES . $category_image->fields['categories_image'])) {
        ...
        if ($https_categories_image = zen_get_https_location(DIR_FS_CATALOG_IMAGES . $category_image->fields['categories_image'])) {
           @unlink($https_categories_image);
        }
      }
    }
...
//      echo 'Products ID: ' . $products->fields['products_id'] . '<br>';
  ...
  function zen_get_https_location($location) {

     if (zen_not_null($location) &&
         defined("DIR_FS_HTTPS_CATALOG") &&
         DIR_FS_HTTPS_CATALOG != '' &&
         DIR_FS_CATALOG != DIR_FS_HTTPS_CATALOG){
       return str_replace(DIR_FS_CATALOG, DIR_FS_HTTPS_CATALOG, $location);
     }
     else {
       return false;
     }
