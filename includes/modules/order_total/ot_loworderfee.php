...
// calculate from flat fee or percentage
    ...
    function install() {
      ...
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('バーチャル商品だけの注文には適応しない', 'MODULE_ORDER_TOTAL_LOWORDERFEE_VIRTUAL', 'false', 'バーチャル商品だけの注文の場合は低額商品取扱い手数料は適応しない。', '6', '8', 'zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
      $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('バーチャルギフト券だけの注文には適応しない', 'MODULE_ORDER_TOTAL_LOWORDERFEE_GV', 'false', 'バーチャルギフト券だけの注文の場合は低額商品取扱い手数料は適応しない。', '6', '9', 'zen_cfg_select_option(array(\'true\', \'false\'), ', now())");
    }
