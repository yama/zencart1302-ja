...
'Apacheの設定によってはファイルのアップロードができなかったりファイルサイズの上限が制限されたりすることがあります。より詳しい説明はApacheの説明を参照してください。');
...
switch ($error_code) {
  case ('1'):
    ...
    define('POPUP_ERROR_TEXT', 'PHPバージョン4.1.2のいくつかのリリースで上位グローバル配列に関するバグが報告されています。このバグによってZen Cartの管理画面が表示されない可能性がありますので、可能であればPHPをアップグレードしてください。<br /><br />PHP 4.2.0以降がZen Cartを実行するための最低ラインです。<br />PHP 4.4.9以降を使用することを強くお勧めします。');
  ...
  case ('21'):
    ...
    define('POPUP_ERROR_TEXT', '持続的接続(Persistent connection)はデータベースの負荷を減少する方法ですが、このオプションを設定する前にサーバ管理者に相談してください。もし\'Persistent connection\'を扱ったことがなければ、データベース上の問題をもたらす可能性があります。');
