...
UPDATE configuration SET configuration_title='大型パッケージの風袋 - 大きさ・重量', configuration_description='大きなパッケージの風袋(ふうたい：大きさと重量)を設定します。<br />\r\n例：10% + 1lb 10:1<br />\r\n10% + 0lbs 10:0<br />\r\n0% + 5lbs 0:5<br />\r\n0% + 0lbs 0:0' WHERE configuration_key='SHIPPING_BOX_PADDING';
...
UPDATE configuration SET configuration_title='低額商品取扱い手数料はバーチャル商品には非適用', configuration_description='カート内がバーチャル商品だけの際に、低額商品取扱い手数料を徴収するかどうかを設定します。' WHERE configuration_key='MODULE_ORDER_TOTAL_LOWORDERFEE_VIRTUAL';
...
UPDATE configuration SET configuration_title='バナー表示グループ - サイドボックス内バナーボックス', configuration_description='どのバナーグループをサイドボックス内バナーボックスに使用しますか? 使わない場合は未記入にします。<br />\r\nデフォルトのグループはSideBox-Bannersです。<br />\r\n<br />\r\nバナー表示グループは1つ(1バナーグループ)または複数(マルチバナーグループ)にすることもできます。マルチバナーグループを表示するためにはグループ名をコロン(<strong>:</strong>)で区切って入力します。<br />\r\n例：Wide-Banners:SideBox-Banners' WHERE configuration_key='SHOW_BANNERS_GROUP_SET7';
UPDATE configuration SET configuration_title='バナー表示グループ - サイドボックス内バナーボックス2', configuration_description='どのバナーグループをサイドボックス内バナーボックスに使用しますか? 使わない場合は未記入にします。<br />\r\nデフォルトのグループはSideBox-Bannersです。<br />\r\n<br />\r\nバナー表示グループは1つ(1バナーグループ)または複数(マルチバナーグループ)にすることもできます。マルチバナーグループを表示するためにはグループ名をコロン(<strong>:</strong>)で区切って入力します。<br />\r\n例：Wide-Banners:SideBox-Banners' WHERE configuration_key='SHOW_BANNERS_GROUP_SET8';
...
UPDATE configuration SET configuration_title='カテゴリ/商品のソート順', configuration_description='カテゴリ/商品のソート順を設定します。<br />0= カテゴリ/商品 ソート順/名前<br />1= カテゴリ/商品 名前<br />2= 商品コード<br />3= 商品数量+, 商品名<br />4= 商品数量-, 商品名<br />5= 商品価格+, 商品名<br />6= 商品価格+, 商品名' WHERE configuration_key='CATEGORIES_PRODUCTS_SORT_ORDER';
UPDATE configuration SET configuration_title='オプション名/オプション値の追加・コピー・削除', configuration_description='オプション名/オプション値の追加・コピー・削除の機能についてのグローバルな設定を行います。<br />0= 機能を隠す<br />1= 機能を表示する<br />2= 商品コード' WHERE configuration_key='OPTION_NAMES_VALUES_GLOBAL_STATUS';
...
UPDATE product_type_layout  SET  configuration_title='バーチャル商品のデフォルトステータス - 新商品追加時', configuration_description='新商品を追加する時の、バーチャル商品のデフォルトステータスをONにしますか？' WHERE configuration_title='Product Virtual Default Status - Skip Shipping Address - When adding new products?' AND configuration_description='Default Virtual Product status to be ON when adding new products?';
